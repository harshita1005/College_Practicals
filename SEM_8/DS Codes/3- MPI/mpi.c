...
rank,local_sum); sum=local_sum; for(int i=1;i<4;i++) 
sum=sum+s[i];
printf("final sum = %d\n\n",sum);
}
else 
...
int local_sum=0; for(int i=0;i<5;i++)
{
...
//for install - sudo apt install mpich

//for compile - mpicc mpi.c -o mpi

//for run - mpirun -np 4 ./mpi
